...
const MCP_TOOLS = [
  ...
  },
  {
    ...
    description: 'Return reference-only JSON describing the fixed navbar, floating_actions, and footer theme slots, real storefront shell data, navbar commerce requirements derived from website_type, and the current MCP-managed root CSS. Unless website_type=brand, preserve cart, registration, and login functions in the navbar; their appearance and layout may follow the reference style. Custom slot markup is user-defined and is not automatically bound to contact fields. Call before creating or modifying visual theme elements; edit root_css.current_css and pass the complete CSS back to slimweb_themes_update_root_elements.css.',
    inputSchema: {
      type: 'object',
      properties: {
        site_id: {
  ...
  },
  {
    ...
    description: 'Update the fixed navbar, floating_actions, and footer root fragments plus theme-level CSS. Unless website_type=brand, navbar markup must preserve functional cart, registration, and login entries; a reference site may change their appearance and layout but not remove them. Add custom content to a slot only when the user explicitly requests it for that slot; data supplied for another placement does not authorize reusing it here. Custom fragment HTML is rendered as supplied; do not auto-bind it to contact data or invent missing links. Theme JavaScript and theme-level libraries are not supported. Do not use this to overwrite page body content. The css field replaces the MCP-managed root-elements CSS file, so include every MCP-managed root style that should remain, including footer/background rules.',
    inputSchema: {
      type: 'object',
      properties: {
        site_id: {
        ...
        fragments: {
          type: 'object',
          description: 'Optional root element HTML fragments keyed only by navbar, floating_actions, or footer.'
        },
        css: {
          type: 'string',
