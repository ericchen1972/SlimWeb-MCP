...
test('MCP tools list includes homepage editing contract tools', async () => {
  await withServer(async (baseUrl) => {
    ...
    assert.equal(toolsByName.get('slimweb.theme_shell.get_context').inputSchema.required.includes('theme_id'), true);
    assert.equal(toolsByName.get('slimweb.themes.update_root_elements').inputSchema.required.includes('theme_id'), true);
    assert.equal(toolsByName.get('slimweb.theme_style_profile.upsert').inputSchema.required.includes('theme_id'), true);
...
test('homepage editing tools call repository implementations', async () => {
  ...
  await withServerOptions({
    ...
    assert.equal((await callTool(26, 'slimweb.theme_shell.get_context', { site_id: 101, theme_id: 22 })).result.content[0].json.reference_only, true);
    assert.equal((await callTool(27, 'slimweb.theme_style_profile.get', { site_id: 101, theme_id: 22 })).result.content[0].json.profile.summary, '童趣');
    assert.equal((await callTool(28, 'slimweb.theme_style_profile.upsert', { site_id: 101, theme_id: 22, summary: '童趣' })).result.content[0].json.ok, true);
    assert.equal((await callTool(29, 'slimweb.theme_style_profile.append_request', { site_id: 101, theme_id: 22, request: '加手繪星星' })).result.content[0].json.ok, true);
    assert.equal((await callTool(30, 'slimweb.themes.update_root_elements', { site_id: 101, theme_id: 22, fragments: { navbar: '<nav>cute</nav>' } })).result.content[0].json.ok, true);
    assert.match((await callTool(31, 'slimweb.preview.get_page_url', { site_id: 101, page_key: 'index' })).result.content[0].json.url, /mcp_page_key=index/);
    assert.equal((await callTool(32, 'slimweb.pages.get_home_content', { site_id: 101 })).result.content[0].json.content.html, '<section>Home</section>');
    assert.equal((await callTool(33, 'slimweb.pages.update_home_content', { site_id: 101, content: { html: '<section>New</section>' } })).result.content[0].json.ok, true);
    assert.match((await callTool(34, 'slimweb.assets.upload', {
      site_id: 101,
    ...
    assert.deepEqual(calls.map((call) => call[0]), ['select', 'themes_list', 'themes_create', 'shell_context', 'profile_get', 'profile_upsert', 'profile_append', 'themes_root', 'preview', 'get_home', 'update_home', 'upload']);
    assert.deepEqual(calls.map((call) => call[1]), [13, 13, 13, 13, 13, 13, 13, 13, 13, 13, 13, 13]);
  });
});
